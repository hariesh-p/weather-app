...
def display(selected, df,avg_df):
    if selected == "Data":
        ...
        check2=st.checkbox("Absolute")
        check1=st.checkbox("Average")
        if check1:
...
# def create_skewt_plot(df):  # Adjust the width and height as needed
...
#         fig = plt.figure()  # Adjust width and height here
#
#         skew = SkewT(fig)
#
#         # Plot wind barbs
...
def comp_display(selected, year_dataframes,avg_dataframes):
    if selected == "Data":
        ...
        check1=st.checkbox("Absolute")
        check2=st.checkbox("Average")
        if check1:
            st.markdown("Absolute Data")
        ...
        if check2:
            ...
            for year, avg_df in avg_dataframes.items():
                st.write(f"Data for {year}")
                st.dataframe(avg_df)
                # plot = px.scatter(avg_dataframes[year], x=xaxis, y=yaxis)
                # plot.add_trace(px.line(df, x=xaxis, y=yaxis, title='Line').data[0])
                plot=go.Figure()
                plot = plot.add_trace(go.Scatter(x=avg_dataframes[year]["Pressure"],
                                               y=avg_dataframes[year]["Avg Temperature"],
                                               name=year))
                st.plotly_chart(plot)
...
if switch:
    ...
    if comoption=="Month":
        ...
        years = st.slider("Select The Years", 1975, ty.year, (2010, 2020))
        ...
        for selected_year in range(years[0], years[1] + 1):
            if monindex == 2:
                if (selected_year % 4 == 0 and selected_year % 100 != 0) or (selected_year % 400 == 0):
                    end_day = 29  # Leap year
                else:
    ...
    box = st.sidebar.selectbox("Choose The Data Source", options=["File", "Date"])
    option = ["Data", "Plots", "Average plots","Wind Plot"]
    ...
    if box == "File":
        ...
        if file is not None:
            ...
            avgs = calculate_averages(dt)
            display(selected,dt, avgs)

    if box == "Date":
        td = datetime.date.today()
        yr = td.year
        mn = td.month
        datetype = st.sidebar.selectbox("Select The Prefered Type", options=["One Day", "Month", "Specific", "Year"])
        ...
        if datetype == "One Day":
            ...
            st.header("Choose the days within the same month")
            ...
            selected_year=st.selectbox("Select Year ",range(1975,yr+1))
